...
public class Puzzles {
		...
		for (int i = 0; i < strArr.length; i++) {
			...
			if (c == ' ') {
				...
				if (i == 0) {
					...
					strArr[j] = '\0';
					/*
					 * Reset i to 0 because there may be more spaces in the
					 * beginning. Here i becomes -1;
					 */
					i--;
				} else {
					count++;
					if (count == 2) {
						/*
						 * Preserve the position of the second space after a
						 * word in 'index'. This is the position from which
						 * extra spaces ahead are covered up.
						 */
						index = i;
					}
				}
				...
				if (count > 1) {
					for (int j = index; (j + (count - 1)) < strArr.length; j++) {
						/*
						 * Cover the extra spaces with next words in the
						 * character array starting from 'index' by shifting
						 * them to left side each time. The next word is at the
						 * distance of (count - 1) from index. Remember, 'index'
						 * is position of the second space after a word.
						 */
						strArr[j] = strArr[j + (count - 1)];
						strArr[j + (count - 1)] = '\0';
					}
					/*
					 * As 'index' is position from which the next word begins,
					 * reset i to index.
					 */
					i = index;
				}
	...
	public static void main(String[] args) {
		...
		System.out
		.println(removeExtraSpaces("   I             am        Amit      Herlekar    "));
